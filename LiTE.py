...
run_startup_checkbox = tk.Checkbutton(checkbox_frame, text="Login when App opens", variable=run_startup_var)
...
def pressSave():
    save_env({"id": username_entry.get(), "password": password_entry.get(), "timeout": timeout_entry.get(), "save_xml":  1 if save_xml_var.get() else 0, "on_start": 1 if run_startup_var.get() else 0})
